    ...
    <div class="row" id="row">
        <div class="col-md-12">
            <form class="form-horizontal" name="student-form" method="POST">
                ...
                <?php
                    $id = $_GET['id'];
                    $query = "SELECT school.name FROM school WHERE school_id = $id";
                    $res1 = mysqli_query($conn, $query);
                    $row = mysqli_fetch_row($res1);

                    echo '<div class="form-group col-sm-3 mb-3">';
                        echo '<label class = "form-label" style="width: 300px;">Are you sure you want to delete school <br><b>' . $row[0] . '?</b></label>';
                        echo '<label class = "form-label" style="width: 300px;">(Note: This action will also delete the library operator and the users associated with this school)</label>';
                    echo '</div>';
                    ...
                    if(isset($_POST['submit_del'])){
                        ...
                        if (mysqli_query($conn, $query)) {
                            header("Location: ./schoolslist.php?error=successdelete");
                            exit();
                        }
                        else{
