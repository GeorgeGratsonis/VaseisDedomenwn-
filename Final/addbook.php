        ...
        <div class="row" id="row">
            ...
            <div class="form-group col-sm-3 mb-3">
                ...
                if (isset($_POST['submit_upd_book'])) {
                    ...
                    if (empty($title) || empty($publisher) || empty($isbn) || empty($pages) || empty($summary) || empty($copies) || empty($image)|| empty($language)|| empty($keywords) || empty($categories)
                    || empty($authorCount) || empty($authorFirstNames) || empty($authorLastNames)) {
                        echo "<hr><span class='error-message'>Please fill in all the fields!</span>";
                    } else if (invalidisbn($isbn) !== false) {
                    ...
                    else if (isbnexists($conn, $isbn, 0) !== false) {
                        echo "<hr><span class='error-message'>ISBN already exists!</span>";
                    } 
                    else if (titleexists($conn, $title, 0) !== false) {
                        echo "<hr><span class='error-message'>Book Title already exists!</span>";
                    } 
                    else{
                        $query = "INSERT INTO Book (Title, Publisher, ISBN, Number_of_pages, Summary, Available_copies, Image, Language, Keywords)
                        VALUES ('$title', '$publisher', '$isbn', '$pages', '$summary', '$copies', '$image', '$language', '$keywords')";
                        ...
                        echo '<script>window.location.href="./booklist.php?error=successadd";</script>';
                        exit();
                    }
