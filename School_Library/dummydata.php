...
while (TRUE) {
    ...
    $stmt = $conn->prepare("SELECT LibraryOperator_ID, Book_ID FROM book_libraryoperator ORDER BY RAND() LIMIT 1");
    $stmt->execute();
    ...
    $stmt->bind_param("sssiii", $borrowingDate, $returnDate, $returned, $userId, $libraryOperatorId, $bookId);
    $stmt->execute();
    $stmt->close();
